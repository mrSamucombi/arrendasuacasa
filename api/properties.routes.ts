// api/properties.routes.ts (versão final)
// TESTE DE SINCRONIZAÇÃO: VERSÃO 12345
...
const createPropertySchema = z.object({
  ...
  price: z.coerce.number({ // <-- SINTAXE CORRETA
  ...
  bathrooms: z.coerce.number({ 
      required_error: "O número de WCs é obrigatório.",
      invalid_type_error: "O número de WCs deve ser um número."
  }).int().min(0),
        ...
        const updatedProperty = await prisma.$transaction(async (tx) => {
            ...
            await tx.transaction.create({
                data: {
                    ...
                    description: `Reativação do imóvel: ${property.title}`
                }
            });
            return property;
        });
