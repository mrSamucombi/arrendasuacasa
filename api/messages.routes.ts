...
import { Router, Request, Response } from 'express'; // <-- Apenas uma importação
...
router.get('/property/:propertyId', checkAuth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!conversation) {
            ...
            if (property.ownerId === userId) return res.status(400).json({ error: "Não pode iniciar uma conversa sobre o seu próprio imóvel." });
...
// ========================================================================
router.post('/:conversationId', checkAuth, async (req: any, res) => {
    try {
...
// ========================================================================
router.put('/:conversationId/read', checkAuth, async (req: any, res) => {
    const { conversationId } = req.params;
    const userId = req.user.uid;
    try {
