...
export const createPropertySchema = z.object({
  ...
  price: z.coerce.number({ invalid_type_error: "O preço deve ser um número válido." }).positive(),
  bedrooms: z.coerce.number({ invalid_type_error: "O número de quartos deve ser um número." }).int().min(0),
  bathrooms: z.coerce.number({ invalid_type_error: "O número de WCs deve ser um número." }).int().min(0),
  area: z.coerce.number({ invalid_type_error: "A área deve ser um número válido." }).positive(),
  imageUrls: z.string().min(1, { message: "Pelo menos uma imagem é necessária." }), // Espera uma string de URLs separados por vírgula
});
