...
// --- ROTA DE DESATIVAÇÃO ---
router.patch('/:id/deactivate', checkAuth, checkPropertyOwnership, async (req, res) => {
    const { id } = req.params;
    try {
...
// --- ROTA DE REATIVAÇÃO ---
router.patch('/:id/reactivate', checkAuth, checkPropertyOwnership, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ error: 'Não autorizado' });
    ...
    try {
        ...
        if (owner.ascBalance < REACTIVATION_COST)
            return res.status(403).json({ error: `Saldo insuficiente. São necessárias ${REACTIVATION_COST} ASC.` });
...
router.get('/', async (req, res) => {
    try {
        ...
        let whereClause = { status: PropertyStatus.AVAILABLE, };
        if (searchTerm) {
            whereClause.OR = [{ title: { contains: searchTerm, mode: 'insensitive' } }, { address: { contains: searchTerm, mode: 'insensitive' } },];
        }
